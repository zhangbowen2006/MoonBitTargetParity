...
Mooncakes 0.1.0：https://mooncakes.io/docs/zhangbowen2006/moonbit-target-parity<br>
...
核心 API 支持目标矩阵、按声明顺序选参考目标、退出码/stdout/stderr 比较、明确的换行规范、可选的 JSON 结构比较、缺失/重复/未声明目标诊断，以及多场景报告聚合。JSON 对象成员顺序可忽略，数组顺序保留。CLI 为 pass/divergent/inconclusive 返回 0/1/2，便于集成 CI。仓库提供真实 MoonBit 探针，脚本分别执行四种目标并将结果交回 MoonBit 比较器。
...
源代码采用 Apache-2.0，仅依赖工具链 core；Mooncakes 0.1.0 已发布。CI 运行严格检查、全目标构建/测试、格式、公共接口快照、CLI 负向路径和真实四目标探针。请评审本项目是否以 MoonBit 生态用户问题为主体；目标矩阵在本机不可用时会如实标记证据不完整。
