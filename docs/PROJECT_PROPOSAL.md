...
源代码采用 Apache-2.0，仅依赖工具链 core；Mooncakes 0.2.0 已发布并通过公开 manifest 核验。CI 运行严格检查、全目标构建/测试、格式、公共接口快照、CLI 负向路径和真实四目标探针。请评审本项目是否以 MoonBit 生态用户问题为主体；目标矩阵在本机不可用时会如实标记证据不完整。
