...
1. JSON 矩阵配置声明 package 路径、相同场景的参数/stdin、目标列表和比较策略；portable corpus 预置整数、Unicode/UTF-8、数组/Map、嵌套 JSON、Result/Option、浮点六类多后端场景。
...
6. JSON/Markdown/自包含 HTML 报告与 CLI 退出码由同一核心结果导出。

## 正确性策略

- exit code 与 stdout 永远比较。
- 提交 expectations 后要求覆盖每个目标；基线不完整时不能放行。这样可以检测所有后端共同产生、但偏离已接受结果的回归。
- ANSI SGR 去除与行尾空白裁剪均为 opt-in 归一化；默认保留原输出，避免把真实差异隐去。
